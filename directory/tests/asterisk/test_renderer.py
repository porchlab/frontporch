...
class AsteriskConfigRendererTests(SimpleTestCase):
    ...
    def test_landline_caller_gets_restricted_extension_context(self):
        ...
        self.assertIn(" same => n,WaitExten(10)", content)
        self.assertIn("exten => 101,1,Dial(PJSIP/alex,30,r(ring))", content)
        self.assertIn("exten => 102,1,Dial(PJSIP/emma,30,r(ring))", content)
        self.assertIn("exten => _X!,1,Hangup(21)", content)
    ...
    def test_landline_caller_can_dial_shortcut_or_approved_four_digit_extension(self):
        ...
        self.assertIn(
            "exten => 1,1,Dial(PJSIP/rowan-phone&PJSIP/rowan-softphone,30,r(ring))",
            content,
        )
        self.assertIn(
            "exten => 3552,1,Dial(PJSIP/rowan-phone&PJSIP/rowan-softphone,30,r(ring))",
            content,
        )
        self.assertIn("exten => 4663,1,Dial(PJSIP/quinn-phone,30,r(ring))", content)
        self.assertIn("exten => 3,1,Dial(PJSIP/quinn-phone,30,r(ring))", content)
    ...
    def test_landline_caller_with_one_target_routes_directly(self):
        ...
        self.assertIn(
            "same => n(approved-landline-12025550199-1),Dial(PJSIP/alex,30,r(ring))",
            content,
        )
    ...
    def test_landline_menu_uses_external_child_landline_fallback(self):
        ...
        self.assertIn(
            " same => n,Dial(PJSIP/13105550100@voipms-endpoint,30,r(ring))",
            context,
        )
    ...
    def test_landline_caller_rings_shared_extension_devices_directly(self):
        ...
        self.assertIn(
            ...
            "PJSIP/alex&PJSIP/alex-linphone,30,r(ring))",
            content,
        )
        self.assertNotIn("[frontporch-landline-inbound-1-1]", content)
