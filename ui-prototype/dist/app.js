...
function migrateDemo(data) {
  for (const child of data.children) child.shortcuts ||= [];
  data.guardians ||= [];
...
const seed = () => ({
  family: 'Maple', parent: 'Morgan', email: 'morgan@example.com', outsideEnabled: false, directoryListed: false, inviteCode: 'MAPLE-9Q4T', guardians: [], guardianInvites: [],
  children: [
  ...
  contacts: [{id:'grandma',name:'Grandma June',phone:'+12025550142',relation:'Grandparent',color:'peach',outgoing:['casey'],incoming:[]},{id:'dad',name:'Drew’s mobile',phone:'+12025550163',relation:'Parent',color:'green',outgoing:['casey','jordan'],incoming:['casey']}],
...
let state = seed();
try { const saved = JSON.parse(sessionStorage.getItem(STORAGE_KEY)); if ([1,2,3,4,5].includes(saved?.version) && saved.data?.children) state = migrateDemo(saved.data); } catch { /* A fresh demo is always available. */ }
...
function save(message) { if (message) state.activity.unshift({text:message,time:'Just now'}); state.activity=state.activity.slice(0,12); try { sessionStorage.setItem(STORAGE_KEY,JSON.stringify({version:5,data:state})); } catch { toast('Changes last until this page is refreshed.'); } }
let toastTimer;
...
function setupChecklist() {
  ...
  return `<section class="setup-banner"><div class="setup-banner-intro"><span class="setup-icon">${icon('home')}</span><div><h2>${count===3?'Your front porch is taking shape.':'A few little steps. A lot of connection.'}</h2><p>${count===3?'Keep your family’s phones and trusted circle up to date.':'Let’s get your family ready for their first hello.'}</p></div><span class="setup-count">${count} of 3 complete</span></div><div class="setup-steps"><button data-action="settings"><span class="step-number done">${icon('check')}</span><span>Make it your family<strong>${esc(state.family)} family is set up</strong></span></button><button data-action="children"><span class="step-number ${phoneDone?'done':''}">${phoneDone?icon('check'):'2'}</span><span>Add their phones<strong>${phoneDone?'A phone for every child':'Give each child a place to call'}</strong></span>${icon('chevron')}</button><button data-action="circle"><span class="step-number ${circleDone?'done':''}">${circleDone?icon('check'):'3'}</span><span>Build your circle<strong>${circleDone?'Your first connection is here':'Invite a family you know'}</strong></span>${icon('chevron')}</button></div></section>`;
}
function childCard(child) {
  const families=state.families.filter(f=>f.connections.some(c=>c.childId===child.id&&c.status==='approved')).length;
  const contacts=state.outsideEnabled?state.contacts.filter(c=>c.outgoing.includes(child.id)).length:0;
...
function contactsPage() { return `<section class="outside-banner"><div class="icon-tile">${icon('phone')}</div><div><h2>Outside calling is ${state.outsideEnabled?'enabled':'off'}</h2><p>${state.outsideEnabled?'Only explicitly approved children and contacts can call each other.':'You can save contacts now. Calls stay blocked until you enable outside calling.'}</p></div><button class="switch" role="switch" aria-checked="${state.outsideEnabled}" aria-label="Enable outside calling" data-action="outside-toggle"><span></span></button></section><div class="contacts-table-wrap">${state.contacts.length?`<table class="contacts-table"><thead><tr><th scope="col">Contact</th><th scope="col">Children can call</th><th scope="col">Can call these children</th><th scope="col"><span class="sr-only">Manage</span></th></tr></thead><tbody>${state.contacts.map(c=>`<tr><th scope="row"><div class="person-line">${avatar(c.name,c.color,'small-avatar')}<div><strong>${esc(c.name)}</strong><span>${esc(c.relation)} · ${formatPhone(c.phone)}</span></div></div></th><td>${c.outgoing.length?esc(names(c.outgoing)):'No children approved'}${!state.outsideEnabled?'<small class="blocked-label">Calling is off</small>':''}</td><td>${c.incoming.length?esc(names(c.incoming)):'No children approved'}${!state.outsideEnabled?'<small class="blocked-label">Calling is off</small>':''}</td><td><button class="text-button blue-text" data-action="contact" data-id="${c.id}">Edit <span class="sr-only">${esc(c.name)}</span>${icon('chevron')}</button></td></tr>`).join('')}</tbody></table>`:emptyState('book','Keep a familiar voice close.','Add a grandparent, parent’s mobile, or another trusted person.','Add a contact','contact')}</div><div class="section-footnote">${icon('lock')} Contact names are private to your family. Incoming and outgoing permissions are managed separately.</div>`; }
function settingsPage() { return `<div class="settings-grid"><section class="panel settings-panel"><div class="section-heading"><h2>Your family</h2>${actionButton('Edit details','edit-family','','secondary','edit')}</div><div class="family-identity">${avatar(state.family,'blue')}<div><h3>${esc(state.family)} family</h3><p>Other parents see this name on your listing and invitations.</p></div></div><dl class="details-list"><div><dt>Family name</dt><dd>${esc(state.family)}</dd></div><div><dt>Primary guardian</dt><dd>${esc(state.parent)}</dd></div><div><dt>Email</dt><dd>${esc(state.email)}</dd></div></dl></section>${guardiansPanel()}${directorySettings()}<section class="safety-panel"><span class="icon-tile">${icon('info')}</span><div><h3>Keep another phone for emergencies.</h3><p>FrontPorch does not support emergency calls. Your family needs another way to reach emergency services.</p></div></section></div>`; }
...
function shortcutTargets(childId) {
  ...
  return [
    ...
    ...(state.outsideEnabled?state.contacts.filter(c=>c.outgoing.includes(childId)).map(c=>({key:JSON.stringify(['contact',c.id]),name:c.name,detail:c.phone,group:'External contacts'})):[]),
  ];
}
function shortcutStatus(childId,shortcut) {
  ...
  try {
    ...
    if(kind==='contact') {
      ...
      reason=!contact?'Contact removed':!contact.outgoing.includes(childId)?'Calling permission removed':!state.outsideEnabled?'Outside calling is off':'Contact unavailable';
...
function contactDialog(id) {const c=state.contacts.find(c=>c.id===id);openDialog(c?'A familiar voice.':'Who’s in your extended circle?',`<p class="dialog-intro">Save a trusted person’s number. Their contact name is private to your family.</p>${textField('Contact name','name',c?.name||'','required maxlength="80" placeholder="e.g. Grandma June"')}<div class="form-row">${textField('Phone number','phone',c?.phone||'','required type="tel" placeholder="+1 202 555 0142"')}<label class="field"><span>Relationship</span><select name="relation">${['Grandparent','Parent','Relative','Family friend','Other'].map(r=>`<option ${c?.relation===r?'selected':''}>${r}</option>`).join('')}</select></label></div><p class="form-note">Use a country code for numbers outside the US or Canada.</p><fieldset><legend>Which children can call this contact?</legend>${childChecks('outgoing',c?.outgoing)}</fieldset><fieldset><legend>Which children can this contact call?</legend>${childChecks('incoming',c?.incoming)}</fieldset><div class="info-strip">${icon('shield')}<p>${state.outsideEnabled?'Only the selected children are approved. Unselected children remain blocked.':'Outside calling is off. These permissions take effect only after you enable it.'}</p></div>${c?`<button type="button" class="text-button danger-text" data-action="delete-contact" data-id="${c.id}">Remove contact</button>`:''}`,{form:'contact',submit:c?'Save contact':'Add contact',extra:`data-id="${id||''}"`}); }
...
document.addEventListener('click', event=>{
  ...
  switch(action){
    ...
    case 'assign-extension':{const used=new Set(state.children.map(c=>c.extension));let n=5249;while(used.has(String(n)))n++;$('[name="extension"]',$('#dialog')).value=String(n);break;}
    ...
    case 'delete-contact':confirmDialog('Remove this contact?','This removes the saved contact and all of their incoming and outgoing calling permissions.','confirm-delete-contact',id);break;
    case 'confirm-delete-contact':state.contacts=state.contacts.filter(c=>c.id!==id);finish('Contact removed. Their calling permissions are revoked.');break;
    ...
    case 'outside-toggle':openDialog(state.outsideEnabled?'Turn outside calling off?':'Enable outside calling?',`<p class="dialog-intro">${state.outsideEnabled?'All calls between your children and external contacts will be blocked. Saved contacts and permissions will be kept.':'Only the children and directions explicitly approved for each contact will be enabled. Unknown callers and unapproved numbers stay blocked.'}</p><div class="demo-notice">This only changes the prototype. Public phone service is not connected.</div><div class="confirm-actions">${actionButton('Cancel','close')}${actionButton(state.outsideEnabled?'Turn off':'Enable approved calls','confirm-outside','','')}</div>`);break;
    case 'confirm-outside':state.outsideEnabled=!state.outsideEnabled;finish(`Outside calling ${state.outsideEnabled?'enabled for approved contacts':'turned off'}.`);break;
...
document.addEventListener('submit',event=>{
  ...
  switch(form.dataset.form){
    ...
    case 'setup-family':{state={...seed(),family:val('family'),parent:signupDraft?.parent||'Morgan',email:signupDraft?.email||'morgan@example.com',directoryListed:data.has('directoryListed'),inviteCode:`FP-${uid().replaceAll('-','').slice(0,8).toUpperCase()}`,children:[],families:[],invites:[],contacts:[],activity:[]};signupDraft=null;save('Your family is set up. Welcome home!');closeDialog();go('overview');toast('Your family is set up. Add your first child next.');break;}
    ...
    case 'contact':{let number=val('phone').replace(/[\s().-]/g,'');if(/^\d{10}$/.test(number))number='+1'+number;else if(/^1\d{10}$/.test(number))number='+'+number;if(!/^\+[1-9]\d{7,14}$/.test(number)){formError(form,'Enter a phone number with country code, such as +1 202 555 0142.');return;}if(state.contacts.some(c=>c.id!==id&&c.phone===number)){formError(form,'This phone number is already in your family contacts. Edit that contact to change its permissions.');return;}let c=state.contacts.find(c=>c.id===id);const updated={name:val('name'),phone:number,relation:val('relation'),outgoing:selected('outgoing'),incoming:selected('incoming')};if(c)Object.assign(c,updated);else state.contacts.push({...updated,id:uid(),color:'peach'});finish(`${updated.name} saved to your private contacts.`);break;}
