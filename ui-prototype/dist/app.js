...
function shortcutsPage() {
  ...
  return `${heading(`Dial shortcuts · ${esc(source.phone.name)}`, "A familiar voice, one digit away.", link("Back to child", `child/${source.child.id}`, "button secondary"))}<div class="info-strip"><p>Shortcuts never add calling permission. Only approved destinations can be assigned.</p></div><div class="dial-grid">${PORTAL.forms.shortcut.find(f => f.name === "digits").choices.map(([digit]) => {
  ...
  try {
    switch (kind) {
      ...
      case "join": {
        ...
        const email = form.dataset.mode === "login" ? String(data.get("username") || "") : invitation?.email;
