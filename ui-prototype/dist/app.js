...
const seed = () => ({
  family: 'Maple', setupDismissed:false, emergencyNoticeDismissed:false, parent: 'Morgan', email: 'morgan@example.com', contactExtensions:{'+12025550142':'6100','+12025550163':'6101'}, directoryListed: false, inviteCode: 'MAPLE-9Q4T', guardians: [], guardianInvites: [],
  children: [
...
function render() { $('#emergency-notice').hidden=state.emergencyNoticeDismissed===true; $('#app').innerHTML=screen==='landing'?landing():familyPage(); document.title=screen==='landing'?'FrontPorch · A little more hello':`${state.family} family · FrontPorch`; }
function familyPage() {
...
document.addEventListener('click', event=>{
  ...
  switch(action){
    ...
    case 'dismiss-emergency-notice':{state.emergencyNoticeDismissed=true;save();render();const main=$('#main');main?.setAttribute('tabindex','-1');main?.focus();toast('911 notice dismissed.');break;}
    case 'show-emergency-notice':state.emergencyNoticeDismissed=false;save();render();window.scrollTo(0,0);$('#emergency-notice')?.focus();break;
